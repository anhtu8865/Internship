...
const ScreenCustomFieldExcerpt = ({ screenCustomField, deleteRow }) => {
  ...
  function deleteConfirm(e, Id) {
    e.preventDefault()
    dispatch(deleteScreenCustomField({ Id })).then((res) =>
      deleteRow(res.payload.Data.Id)
    )
    //window.location.reload(false);
  }
  return (
    <tr key={screenCustomField.Id}>
      ...
      <td className="px-5 py-5 text-center border-b border-gray-200 bg-white text-sm">
        <span className="relative inline-block px-3 ml-1.5 py-1 font-semibold text-green-900 leading-tight">
          ...
          />
          <a
            onClick={(e) => deleteConfirm(e, screenCustomField.Id)}
...
export const ScreenCustomFields = () => {
  ...
  //console.log(listCustomFields, 'kkkkkkkkkkkkk')
  const dispatch = useDispatch()
  ...
  //console.log(temp, 'kkkkkkkkkkkkk')
  ...
  let tbody = listCustomFields?.map((screenCustomField) => (
